...
const companyNav = [
    ...
    { icon: Newspaper, label: "Press & Media", sub: "News and media coverage", href: "/company#press" },
...
const navItems = [
    ...
    { name: "Company", href: "/company", hasMega: true },
    { name: "Resources", href: "/resources" },
    { name: "Get In Touch", href: "/contact" },
];

/* ─────────────────────────────────────────────
   ANIMATED LETTER LINK
───────────────────────────────────────────── */
...
function AnimatedLink({ name, href, className = "" }: { name: string; href: string; className?: string }) {
    return (
        <Link
            href={href}
            className={`group text-gray-800 dark:text-gray-200 text-md font-medium flex overflow-hidden ${className}`}
        >
            {name.split("").map((c, i) => (
                <span
                    key={i}
            ...
            group-hover:from-indigo-400 group-hover:to-indigo-500
...
function CompanyMegaMenu({ visible }: { visible: boolean }) {
    return (
        <div
            className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[640px] z-50
        ...
        ${visible ? "opacity-100 scale-y-100 pointer-events-auto" : "opacity-0 scale-y-95 pointer-events-none"}`}
            style={{ clipPath: visible ? "none" : undefined }}
        >
            {/* Header strip */}
            <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                ...
                <Link
                    href="/company"
                    className="text-[11px] font-bold text-gray-400 hover:text-indigo-500 transition-colors tracking-wide"
                >
                    View all →
                </Link>
            </div>
...
export default function Navbar() {
    ...
    function handleMouseLeave() {
        timeoutRef.current = setTimeout(() => setDesktopCompanyOpen(false), 120);
    }
                ...
                <div className="hidden md:flex items-center gap-8">
                    {navItems.map((item) =>
                        ...
                        dark:group-hover:from-indigo-300 dark:group-hover:to-indigo-400"
                                    ...
                                    {/* Chevron */}
                                    <svg
                                        className={`w-3 h-3 ml-1 flex-shrink-0 text-gray-400 transition-transform duration-200 ${companyOpen ? "rotate-180" : ""}`}
                                ...
                                <CompanyMegaMenu visible={desktopCompanyOpen} />
                            </div>
                        ) : (
